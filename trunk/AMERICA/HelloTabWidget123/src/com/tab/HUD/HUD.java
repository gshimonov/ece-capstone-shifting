...
//					// value is the value from the arduino
					...
					if ( (buffer[i+4] & 0x80) > 0){
						flag2=1;
					}
					
					buffer[i+1]= (byte) (buffer[i+1] & (0x7F));
					buffer[i+4]= (byte) (buffer[i+4] & (0x7F));
					
					int speed = ( ((int) buffer[i]) << 8);
					speed += ( ((int) buffer[i+1]));
					int gear = (int) buffer[i+2];
					int cadence = ( ((int) buffer[i+3]) << 8);
					cadence += (int) buffer[i+4];
					
					speed = speed + (flag << 7);
					cadence = cadence + (flag2 << 7);
